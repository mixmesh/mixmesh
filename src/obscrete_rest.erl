...
wait_for_serv(Name, Timeout) when is_atom(Name), is_integer(Timeout) ->
    N = Timeout div 100,
    wait_for_serv_(Name, N, 100).

wait_for_serv_(_Name, 0, _Timeout) ->
    false;
...
do_start(Start) ->
    ...
    %% wait for log server to start max 5000
    wait_for_serv(log_serv, 5000),

    Opts = [HttpAddress, HttpCertFilename, HttpPassword] =
	config:lookup_children([address, 'cert-filename', password],
			       config:lookup(['http-server'])),
    Nym = config:lookup([player,nym]),
    {IfAddr,Port} = HttpAddress,
    ?dbg_log_fmt("starting with ~p", [Opts]),
